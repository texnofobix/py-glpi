...
class Problem(GlpiItem):
    """ Object of Item Problem """
    ...
    def __init__(self, name=None, content=None, attributes={}):
        ...
        if name is None or content is None:
            raise GlpiInvalidArgument(
                'Cannot open a problem without Name and Content data')
        else:
            """ Define name and content, required to every new one problem.  """
...
class GlpiProblem(GlpiService):
    """ Client for GLPI Problem item """
    ...
    """ CREATE """
    def new(self, name=None, content=None, problem_data=None):
        """
        This is wrapper of polymorphic create() method.
        new() will called when an object Problem is not create with name set up.
        In general we advise to use create().
        """

        if (name is None or content is None) and problem_data is None:
            return "{ 'error_message' : 'Name or content not found.'}"

        problem = None
        if problem_data is None:
            problem = Problem(name, content)
        else:
            problem = problem_data

        return self.create(problem)
